...
function SpotlightCard({ label, colorKey, stocks, onOpen }) {
  ...
  return (<div style={{ borderRadius: 12, padding: 16, background: col.bg, border: `1px solid ${col.bdr}` }}><div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 12 }}><div style={{ width: 7, height: 7, borderRadius: '50%', background: col.c, flexShrink: 0 }} /><span style={{ fontFamily: 'DM Mono,monospace', fontSize: 9, letterSpacing: 1.5, textTransform: 'uppercase', fontWeight: 500, color: col.c }}>{label}</span></div>{stocks.length === 0 ? <div style={{ fontSize: 11, color: G.txt3, fontStyle: 'italic' }}>Sem posições.</div> : <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>{stocks.map(x => (<div key={x.t} onClick={() => onOpen(x.t)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', borderRadius: 8, background: 'rgba(255,255,255,.8)', border: '1px solid rgba(255,255,255,.95)', cursor: 'pointer', transition: 'all .13s' }} onMouseEnter={e => { e.currentTarget.style.background = '#fff'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(0,0,0,.1)'; }} onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,.8)'; e.currentTarget.style.boxShadow = 'none'; }}><div><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 11, fontWeight: 500, color: col.c }}>{x.t}</div><div style={{ fontSize: 10, color: G.txt3, marginTop: 1 }}>{x.n.split(' ')[0]}</div></div><span style={{ fontFamily: 'DM Mono,monospace', fontSize: 10, fontWeight: 500, color: col.c }}>{x.pnl >= 0 ? '+' : ''}{x.pp.toFixed(1)}%</span></div>))}</div>}</div>);
}
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(26,23,20,.4)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => { if (e.target === e.currentTarget && !running) onClose(); }}>
      <div style={{ background: G.surf, borderRadius: 16, padding: 28, width: 480, maxHeight: '80vh', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ fontSize: 12, color: G.txt2 }}>{unrated.length} stock(s) sem rating. Análise com DeepSeek — ~3s entre cada um.{unrated.length === 0 && <span style={{ color: G.sb.c }}> Todos já têm rating!</span>}</div>
        {unrated.length > 0 && (<><div style={{ background: G.surf2, borderRadius: 8, height: 6, overflow: 'hidden' }}><div style={{ background: G.b.c, height: '100%', width: `${pct}%`, transition: 'width .3s' }} /></div><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 10, color: G.txt3 }}>{progress}/{unrated.length} — {pct}%</div></>)}
        ...
        {!running && !done && unrated.length > 0 && (<button onClick={run} style={{ padding: '10px 20px', background: G.txt, border: 'none', color: G.bg, fontSize: 13, fontWeight: 500, cursor: 'pointer', borderRadius: 9 }}>Iniciar Análise</button>)}
...
export default function ApexApp() {
  ...
  const tabItems = [{ id: 'portfolio', label: 'Portfolio' }, { id: 'insider', label: 'Insiders' }, { id: 'news', label: 'Notícias' }, { id: 'overview', label: 'Overview' }];
...
function InsiderPanel({ tickers }) {
  const [rows, setRows] = useState(null); const [error, setError] = useState(null);
  useEffect(() => { let cancelled = false; const results = []; let idx = 0; function next() { if (cancelled) return; if (idx >= tickers.length) { if (!cancelled) setRows(results.sort((a, b) => new Date(b.transactionDate || b.filingDate) - new Date(a.transactionDate || a.filingDate))); return; } const t = tickers[idx++]; fetch(`/api/insiders?symbol=${t}`).then(r => r.json()).then(d => { if (Array.isArray(d)) d.forEach(i => results.push({ ...i, ticker: t })); }).catch(() => {}).finally(() => setTimeout(next, 130)); } next(); return () => { cancelled = true; }; }, []);
  if (!rows) return <Spinner />;
  if (error) return <div style={{ padding: 14, background: G.ss.bg, border: `1px solid ${G.ss.bdr}`, borderRadius: 10, color: G.ss.c, fontSize: 12 }}>{error}</div>;
  if (!rows.length) return <div style={{ padding: 14, background: G.na.bg, border: `1px solid ${G.na.bdr}`, borderRadius: 10, color: G.na.c, fontSize: 12 }}>Sem dados de insiders.</div>;
  return (<div style={{ display: 'grid', gap: 4 }}>{rows.map((d, i) => { const buy = (d.acquistionOrDisposition || d.transactionType || '').toUpperCase().startsWith('A'); const amt = d.securitiesTransacted && d.price ? (d.price * d.securitiesTransacted).toFixed(0) : (d.value || '?'); return (<div key={i} style={{ display: 'grid', gridTemplateColumns: '65px 1fr 110px 80px 85px', alignItems: 'center', padding: '11px 13px', background: G.surf, border: `1px solid ${G.bdr}`, borderRadius: 10, gap: 10 }}><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 12, fontWeight: 500 }}>{d.ticker}</div><div><div style={{ fontSize: 12 }}>{d.reportingName || d.name || 'N/A'}</div><div style={{ fontSize: 10, color: G.txt3, marginTop: 2 }}>{d.typeOfOwner || d.position || 'Insider'}</div></div><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 12, textAlign: 'right', color: buy ? G.sb.c : G.ss.c }}>€{Number(amt).toLocaleString('pt-PT')}</div><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 10, textAlign: 'center', fontWeight: 500, color: buy ? G.sb.c : G.ss.c }}>{buy ? '▲ Compra' : '▼ Venda'}</div><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 10, color: G.txt3, textAlign: 'right' }}>{(d.transactionDate || d.filingDate || '').split('T')[0]}</div></div>); })}</div>);
}

function NewsPanel({ tickers }) {
  const [items, setItems] = useState(null);
  useEffect(() => { let cancelled = false; const results = []; let idx = 0; function next() { if (cancelled) return; if (idx >= tickers.length) { if (!cancelled) setItems(results.sort((a, b) => (b.datetime || 0) - (a.datetime || 0))); return; } const t = tickers[idx++]; fetch(`/api/news?symbol=${t}`).then(r => r.json()).then(d => { if (Array.isArray(d)) d.forEach(n => results.push({ ...n, ticker: t })); }).catch(() => {}).finally(() => setTimeout(next, 90)); } next(); return () => { cancelled = true; }; }, []);
  if (!items) return <Spinner />;
  if (!items.length) return <div style={{ padding: 14, background: G.na.bg, border: `1px solid ${G.na.bdr}`, borderRadius: 10, color: G.na.c, fontSize: 12 }}>Sem notícias recentes.</div>;
  return (<div style={{ display: 'grid', gap: 5 }}>{items.map((n, i) => { const sent = n.sentiment > 0.2 ? 'pos' : n.sentiment < -0.2 ? 'neg' : 'neu'; const sc = { pos: G.sb, neg: G.ss, neu: G.na }[sent]; const sl = { pos: 'Bullish', neg: 'Bearish', neu: 'Neutro' }[sent]; const dt = n.datetime ? new Date(n.datetime * 1000).toLocaleDateString('pt-PT') : ''; return (<div key={i} onClick={() => window.open(n.url, '_blank')} style={{ padding: '13px 14px', background: G.surf, border: `1px solid ${G.bdr}`, borderRadius: 10, cursor: 'pointer' }}><div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 9, color: G.txt3, letterSpacing: 1, textTransform: 'uppercase' }}>{n.ticker} · {n.source || 'News'}</div><div style={{ fontFamily: 'DM Mono,monospace', fontSize: 9, color: G.txt3 }}>{dt}</div></div><div style={{ fontSize: 13, lineHeight: 1.5, marginBottom: 5 }}>{n.headline}</div><span style={{ display: 'inline-block', padding: '2px 9px', borderRadius: 20, fontFamily: 'DM Mono,monospace', fontSize: 9, background: sc.bg, color: sc.c }}>{sl}</span></div>); })}</div>);
}
