...
function AnalystPanel({ ticker }) {
  ...
  return (
    <div style={{ marginTop:0 }}>
      ...
      <div style={{ padding:14, background:G.card2, borderRadius:10, border:`1px solid ${G.border}`, marginBottom:12 }}>
        <div style={{ color:G.muted, fontSize:10, marginBottom:10, textTransform:'uppercase', letterSpacing:'.06em' }}>DistribuiÃÂÃÂ§ÃÂÃÂ£o Sentimento</div>
...
function NewsPanel({ tickers }) {
  ...
  const newsItems = [
    ...
    { outlet:'Bloomberg', outletBg:'#3C3489', outletColor:'#EEEDFE', sentiment:'Bearish', sClass:'bear', cat:'Macro', time:'4h ago',
      title:'Fed holds rates, dots signal only one cut in 2025', desc:'Powell mantÃ©m postura hawkish. Mercado reprecia cortes para Q4.' },
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <div style={{background:'rgba(124,106,247,.08)',border:'1px solid rgba(124,106,247,.2)',borderRadius:10,padding:14}}>
        <div style={{fontSize:11,fontWeight:700,color:'var(--acc)',letterSpacing:1,marginBottom:8}}>SÍNTESE AI — ultimas 24h</div>
        <div style={{fontSize:12.5,color:'var(--soft)',lineHeight:1.75}}>
          Mercado em modo de espera pre-Fed. Tech mantÃ©m momentum com resultados acima do esperado.
...
function InsidersPanel({ tickers }) {
  ...
  const V = (s) => ({background:'var(--card)',border:'1px solid var(--bord)',borderRadius:10,padding:12});
  const pill = (bg,col,bord,txt) => (
    <span style={{display:'inline-flex',alignItems:'center',padding:'2px 8px',borderRadius:999,fontSize:11,fontWeight:600,background:bg,color:col,border:'1px solid '+bord}}>{txt}</span>
  );

  return (
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <div style={{background:'var(--card)',border:'1px solid var(--bord)',borderRadius:12,padding:16}}>
        <h2 style={{marginBottom:14}}>Insiders — Volume Total do Mercado</h2>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:12}}>
          <div style={{background:'rgba(34,212,122,.04)',border:'1px solid var(--bord)',borderRadius:10,padding:12}}>
            ...
            <div style={{fontSize:11,color:'var(--muted)'}}>42.318 transações</div>
          </div>
          <div style={{background:'rgba(240,79,90,.04)',border:'1px solid var(--bord)',borderRadius:10,padding:12}}>
            ...
            <div style={{fontSize:11,color:'var(--muted)'}}>89.741 transações</div>
          </div>
        </div>
        ...
        <div style={{height:6,background:'var(--faint)',borderRadius:3,overflow:'hidden',marginBottom:16}}>
          <div style={{height:'100%',width:'37%',background:'var(--green)'}}></div>
        </div>

        <div style={{fontSize:12,fontWeight:700,color:'var(--text)',marginBottom:8}}>
          Top 3 — maior actividade de insiders{' '}
          <span style={{fontSize:10,color:'var(--muted)',fontWeight:400}}>(exclui 10b5-1 programadas)</span>
        </div>
        <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:16}}>
          <div style={{background:'var(--card2)',border:'1px solid var(--bord)',borderRadius:10,padding:12}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:5}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <span style={{fontSize:14,fontWeight:700,color:'var(--acc)'}}>NVDA</span>
                {pill('#f04f5a22','var(--red)','#f04f5a33','Venda líquida')}
              </div>
              <span style={{fontSize:11,color:'var(--muted)'}}>$42M movimentados</span>
            </div>
            <div style={{fontSize:11,color:'var(--muted)',lineHeight:1.4}}>Jensen Huang vendeu $38M (stock grants, não 10b5-1). 2 diretores venderam $4M. Zero compras discricionarías.</div>
          </div>
          <div style={{background:'var(--card2)',border:'1px solid var(--bord)',borderRadius:10,padding:12}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:5}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
                <span style={{fontSize:14,fontWeight:700,color:'var(--acc)'}}>GOOGL</span>
                {pill('#22d47a22','var(--green)','#22d47a33','Compra líquida')}
              </div>
              <span style={{fontSize:11,color:'var(--muted)'}}>$21M movimentados</span>
            </div>
            <div style={{fontSize:11,color:'var(--muted)',lineHeight:1.4}}>Sundar Pichai comprou $8M open market. 4 diretores compraram $13M total. Sinal bullish — compras discricionarías.</div>
          </div>
          <div style={{background:'var(--card2)',border:'1px solid var(--bord)',borderRadius:10,padding:12}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:5}}>
              <div style={{display:'flex',alignItems:'center',gap:8}}>
        ...
        <div style={{background:'var(--card2)',border:'1px solid var(--bord)',borderRadius:10,padding:12}}>
          <div style={{display:'flex',justifyContent:'space-around',marginBottom:10,flexWrap:'wrap',gap:10}}>
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:11,color:'var(--muted)'}}>Volume Compras</div>
              <div style={{fontSize:20,fontWeight:800,color:'var(--green)'}}>$0</div>
              <div style={{fontSize:11,color:'var(--muted)'}}>0 transações reais</div>
            </div>
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:11,color:'var(--muted)',marginBottom:3}}>Buy/Sell Ratio</div>
              <div style={{fontSize:28,fontWeight:900,color:'var(--red)'}}>0%</div>
              <div style={{fontSize:11,color:'var(--muted)'}}>compras reais</div>
            </div>
            <div style={{textAlign:'center'}}>
              <div style={{fontSize:11,color:'var(--muted)'}}>Volume Vendas Reais</div>
              <div style={{fontSize:20,fontWeight:800,color:'var(--red)'}}>$12.4M</div>
              <div style={{fontSize:11,color:'var(--muted)'}}>após filtro 10b5-1</div>
            </div>
          </div>
          <div style={{height:8,background:'var(--faint)',borderRadius:4,overflow:'hidden',marginBottom:8}}>
            <div style={{width:'0%',height:'100%',background:'var(--green)'}}></div>
          </div>
          {loading && <div style={{fontSize:11,color:'var(--muted)',textAlign:'center',padding:8}}>A carregar dados reais...</div>}
          <div style={{display:'flex',gap:8,flexWrap:'wrap',marginTop:6}}>
            <span style={{display:'inline-flex',alignItems:'center',padding:'2px 8px',borderRadius:999,fontSize:10,fontWeight:600,background:'#f5a62322',color:'var(--yel)',border:'1px solid #f5a62333'}}>⚠ 8 vendas 10b5-1 filtradas (automáticas)</span>
            <span style={{display:'inline-flex',alignItems:'center',padding:'2px 8px',borderRadius:999,fontSize:10,fontWeight:600,background:'#f04f5a22',color:'var(--red)',border:'1px solid #f04f5a33'}}>3 vendas discricionarías reais — bearish signal</span>
          </div>
        </div>
      </div>
    </div>
...
function OverviewPanel({ portfolio, ratings }) {
  ...
  const hhiLabel = hhi > 2500 ? 'Alta ConcentraÃÂÃÂ§ÃÂÃÂ£o' : hhi > 1500 ? 'Moderada' : 'Diversificado';
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:18 }}>
      ...
      <Card>
        <div style={{ color:G.text, fontSize:14, fontWeight:700, marginBottom:16 }}>Top 5 PosiÃÂÃÂ§ÃÂÃÂµes</div>
        ...
        <div style={{ borderTop:`1px solid ${G.faint}`, marginTop:12, paddingTop:9, color:G.muted, fontSize:12 }}>
          {portfolio.length - 5} posiÃÂÃÂ§ÃÂÃÂµes restantes
        </div>
      </Card>
      <Card><GrowthChart portfolio={portfolio}/></Card>
      <Card>
        <div style={{ color:G.text, fontSize:14, fontWeight:700, marginBottom:16 }}>DistribuiÃÂÃÂ§ÃÂÃÂ£o Setorial</div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
        {[{ label:'ÃÂ°ÃÂÃÂÃÂ Top Performers', data:winners, color:G.green }, { label:'ÃÂ°ÃÂÃÂÃÂ Piores', data:topLosers, color:G.red }].map(({ label, data, color }) => (
          <Card key={label} style={{ padding:16 }}>
...
function DiscoverPanel({ portfolio }) {
  ...
  return (
    <div>
      ...
      {done && suggestions.length > 0 && (
        <>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
            <div style={{ color:G.muted, fontSize:13 }}>{suggestions.length} sugestÃÂÃÂµes</div>
                ...
                <Card key={i} style={{ padding:20, position:'relative', overflow:'hidden' }}>
                  ...
                  <div style={{ paddingLeft:12 }}>
                    ...
                    <div style={{ padding:12, background:G.card2, borderRadius:9, border:`1px solid ${G.border}` }}>
                      <div style={{ color:G.muted, fontSize:10, marginBottom:8, textTransform:'uppercase', letterSpacing:'.06em' }}>AnÃÂÃÂ¡lise de Probabilidade</div>
      ...
      {done && !suggestions.length && !error && <div style={{ color:G.muted, padding:40, textAlign:'center' }}>Sem sugestÃÂÃÂµes. Tenta regenerar.</div>}
    </div>
  );
}
...
const MACRO_INDICES = [
  ...
  { key:'dxy',     label:'DXY',        color:'#4a9eff', desc:'ForÃÂÃÂ§a do dÃÂÃÂ³lar americano. DXY alto pressiona commodities e mercados emergentes. DXY baixo favorece ouro, crypto e acÃÂÃÂ§ÃÂÃÂµes internacionais.' },
  { key:'wti',     label:'WTI Oil',    color:'#f5a623', desc:'PetrÃÂÃÂ³leo bruto WTI. Proxy de crescimento global e inflaÃÂÃÂ§ÃÂÃÂ£o. >$90 pressiona margens das empresas. Estreito de Ormuz = risco geopolÃÂÃÂ­tico chave.' },
  { key:'eurusd',  label:'EUR/USD',    color:'#00d4c8', desc:'Par euro/dÃÂÃÂ³lar. Reflecte divergÃÂÃÂªncia econÃÂÃÂ³mica EUA vs Europa. Impacta receitas de multinacionais com exposiÃÂÃÂ§ÃÂÃÂ£o europeia.' },
  { key:'yield10', label:'10Y Yield',  color:'#b06ef7', desc:'Yield do tesouro americano a 10 anos. Taxa de desconto para valorizaÃÂÃÂ§ÃÂÃÂ£o de acÃÂÃÂ§ÃÂÃÂµes. >4.5% comprime mÃÂÃÂºltiplos de crescimento (tech especialmente).' },
  { key:'gold',    label:'Gold',       color:'#f5a623', desc:'Ouro como activo refÃÂÃÂºgio. Sobe em perÃÂÃÂ­odos de stress geopolÃÂÃÂ­tico, inflaÃÂÃÂ§ÃÂÃÂ£o e dÃÂÃÂ³lar fraco. CorrelaÃÂÃÂ§ÃÂÃÂ£o negativa com yields reais.' },
  { key:'sp500',   label:'S&P 500',    color:'#22d47a', desc:'Benchmark do mercado americano. TendÃÂÃÂªncia de longo prazo define o contexto macro. 200MA = linha divisÃÂÃÂ³ria bull/bear market.' },
];
...
function MacroPanel() {
  ...
  const fmtChg = (d) => {
    if (!d) return null;
    const pct = d.dp || 0;
    return { pct, up: pct >= 0, str: (pct >= 0 ? 'Ã¢ÂÂ² +' : 'Ã¢ÂÂ¼ ') + Math.abs(pct).toFixed(2) + '%' };
  };
...
export default function MainApexApp() {
  ...
  const tabs = ['Portfolio','Insiders','Notícias','Overview','Descobertas','Crypto'];
  ...
  return (
    <div style={{ background:G.bg, minHeight:'100vh', color:G.text, fontFamily:"'DM Sans',system-ui,sans-serif", padding:'20px 16px' }}>
      <div style={{ maxWidth:920, margin:'0 auto' }}>
        ...
        {activeTab === 'Notícias'    && <NewsPanel tickers={tickers}/>}
