...
function MacroPanel() {
  ...
  const INDICES = [
    { key:'SPY',    label:'S&P 500',     color:'#7c6af7', fmt: v => v.toFixed(0) },
    { key:'QQQ',    label:'NASDAQ',      color:'#7c6af7', fmt: v => v.toFixed(0) },
    { key:'VIX',    label:'VIX',         color:'#f04f5a', fmt: v => v.toFixed(1) },
    { key:'GC=F',   label:'Ouro',        color:'#f5a623', fmt: v => v.toFixed(0) },
    { key:'CL=F',   label:'WTI Oil',     color:'#888780', fmt: v => v.toFixed(1) },
    { key:'DX-Y.NYB', label:'DXY',       color:'#1D9E75', fmt: v => v.toFixed(1) },
    { key:'BTC-USD',  label:'BTC',       color:'#f5a623', fmt: v => '$' + Math.round(v).toLocaleString() },
    { key:'TNX',    label:'T-Note 10Y',  color:'#f04f5a', fmt: v => v.toFixed(2) + '%' },
  ];
  const IMPACT = {
    ...
    'VIX':      'Acima de 25 = regime de alta volatilidade. Abaixo de 15 = complacência. Usa opções se VIX < 15 para hedge barato.',
    'GC=F':     'Ouro em máximos = incerteza geopolítica elevada. Portfólio sem 3-5% em ouro está subexposto neste regime.',
    'CL=F':     'Acima de $90 pressiona margens e inflação. Abaixo de $70 alivia input costs para industriais e transporte.',
    'DX-Y.NYB': 'USD forte comprime receitas internacionais de AAPL, MSFT, GOOGL. USD fraco é tailwind para commodities.',
    'BTC-USD':  'Correlação com Nasdaq ~0.7. BTC acima de $60k com ETF flows positivos = risk-on indicator.',
    'TNX':      'Yield 10Y acima de 4.5% comprime P/E de growth stocks. Cada +50bps = ~-8% no fair value de tech com P/E 30x.',
  };
  useEffect(() => {
    const symbols = INDICES.map(i => i.key);
    let cancelled = false;
    (async () => {
      const result = {};
      for (const sym of symbols) {
        try {
          ...
          if (r.ok) { const d = await r.json(); result[sym] = d; }
        } catch(e) {}
        if (cancelled) return;
      }
      if (!cancelled) setPrices(result);
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: '#888', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Macro Global</div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8 }}>
        {INDICES.map(idx => {
          const d = prices[idx.key];
          const price = d ? idx.fmt(d.c || d.price || 0) : '...';
          const chg = d ? ((d.dp || 0)).toFixed(2) : null;
          const isUp = d && (d.dp || 0) >= 0;
          const isExpanded = expanded === idx.key;
          return (
            <div key={idx.key}
              onClick={() => setExpanded(isExpanded ? null : idx.key)}
              style={{ background: '#0e0e1a', border: isExpanded ? '1px solid ' + idx.color : '1px solid #1e1e2e', borderRadius: 8, padding: '8px 10px', cursor: 'pointer', transition: 'border-color 0.2s' }}>
              <div style={{ fontSize: 10, color: '#888', marginBottom: 2 }}>{idx.label}</div>
              <div style={{ fontSize: 16, fontWeight: 600, color: idx.color }}>{price}</div>
              {chg !== null && (
                <div style={{ fontSize: 10, color: isUp ? '#1D9E75' : '#f04f5a' }}>{isUp ? '+' : ''}{chg}%</div>
              )}
            </div>
          );
        })}
      ...
      {expanded && IMPACT[expanded] && (
        <div style={{ marginTop: 8, padding: '8px 12px', background: '#0e0e1a', border: '1px solid #1e1e2e', borderRadius: 8, fontSize: 12, color: '#aaa', lineHeight: 1.5 }}>
          {IMPACT[expanded]}
        </div>
      )}
    </div>
...
function AIAlerts({ portfolio, ratings }) {
  ...
  useEffect(() => {
    ...
    const totalValue = portfolio.reduce((s, x) => s + (x.shares * (x.pp || 0)), 0);
    const result = [];
    portfolio.forEach(s => {
      const r = ratings[s.t] || {};
      const rating = r.rating || '';
      const currentVal = s.shares * (s.pp || 0);
      const weight = totalValue > 0 ? (currentVal / totalValue) * 100 : 0;
      const pnlPct = s.pp > 0 ? ((s.pp - s.avgCost) / s.avgCost) * 100 : 0;

      // REDUZIR: peso excessivo (>25%) ou rating SELL/STRONG_SELL
      if (weight > 25) {
        result.push({ type: 'REDUZIR', ticker: s.t, msg: 'Concentração de ' + weight.toFixed(1) + '% (limite recomendado: 25%). Reduz para rebalancear risco de portfólio.' });
      ...
      // VIGIAR: drawdown >15% ou rating SELL sem estar em REDUZIR, ou pnl muito positivo com peso alto
      else if (pnlPct < -15) {
        ...
        result.push({ type: 'VIGIAR', ticker: s.t, msg: 'Posição com +' + pnlPct.toFixed(0) + '% de ganho e peso ' + weight.toFixed(1) + '%. Considera realizar lucros parciais.' });
      }
      // ACUMULAR: rating BUY/STRONG_BUY, peso baixo (<12%), e não em perda severa
      else if ((rating === 'BUY' || rating === 'STRONG_BUY') && weight < 12 && pnlPct > -10) {
        result.push({ type: 'ACUMULAR', ticker: s.t, msg: 'Rating AI: ' + rating + '. Peso actual ' + weight.toFixed(1) + '% — espaço para aumentar até 15%. ' + (r.text ? r.text.split('.')[0] + '.' : '') });
      }
    });
  ...
  const colors = { REDUZIR: '#f04f5a', VIGIAR: '#f5a623', ACUMULAR: '#1D9E75' };
  const bg = { REDUZIR: 'rgba(240,79,90,0.08)', VIGIAR: 'rgba(245,166,35,0.08)', ACUMULAR: 'rgba(29,158,117,0.08)' };
  return (
    <div style={{ marginBottom: 16 }}>
      <div style={{ fontSize: 11, fontWeight: 600, color: '#888', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>AI Alerts</div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {alerts.map((a, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '10px 12px', borderRadius: 8, background: bg[a.type], border: '1px solid ' + colors[a.type] + '33' }}>
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 7px', borderRadius: 4, background: colors[a.type], color: '#fff', whiteSpace: 'nowrap', marginTop: 1 }}>{a.type}</span>
            <div>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#e8e8f0', marginBottom: 2 }}>{a.ticker}</div>
              <div style={{ fontSize: 12, color: '#999', lineHeight: 1.4 }}>{a.msg}</div>
            </div>
          </div>
        ))}
      </div>
