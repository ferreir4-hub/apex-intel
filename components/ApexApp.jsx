...
function AnalystPanel({ticker}){
  ...
  return<div style={{marginTop:16,padding:16,background:'#0d0d1a',borderRadius:10,border:`1px solid ${G.border}`}}>
    <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
      <span style={{color:G.muted,fontSize:12}}>Wall Street Consensus</span><Pill color={meta.color} bg={meta.bg}>{meta.label}</Pill>
...
function AnalysisPanel({stock,onClose}){
  const[analysis,setAnalysis]=useState(null);const[loading,setLoading]=useState(true);const[error,setError]=useState(null);
  useEffect(()=>{fetch('/api/batch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({stocks:[stock]})}).then(r=>r.json()).then(d=>{const result=d.results?.[0];if(result?.error)setError(result.error);else setAnalysis(result);setLoading(false);}).catch(e=>{setError(e.message);setLoading(false);});},[stock.ticker]);
  ...
  return<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.85)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}}>
    <div style={{background:G.card,border:`1px solid ${G.border}`,borderRadius:16,padding:28,maxWidth:700,width:'100%',maxHeight:'85vh',overflowY:'auto',position:'relative'}}>
      ...
      <div style={{marginBottom:20}}>
        <div style={{display:'flex',alignItems:'center',gap:12}}><h2 style={{margin:0,color:G.text,fontSize:22}}>{stock.ticker}</h2><span style={{color:G.muted,fontSize:14}}>{stock.name}</span></div>
      ...
      {analysis&&!loading&&<div style={{marginTop:20}}>
...
async function runBatch(stocks,setRatings){
  try{const res=await fetch('/api/batch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({stocks})});const data=await res.json();if(data.results){const map={};data.results.forEach(r=>{if(r.ticker&&r.rating)map[r.ticker]=r.rating;});setRatings(map);await fetch('/api/ratings',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({ratings:map});});}}catch(e){console.error('Batch error:',e);}
}
function BatchAnalyser({stocks,ratings,setRatings}){
  const[running,setRunning]=useState(false);const[done,setDone]=useState(false);const hasRun=useRef(false);
  const handleRun=useCallback(async()=>{if(hasRun.current||running)return;hasRun.current=true;setRunning(true);await runBatch(stocks,setRatings);setRunning(false);setDone(true);},[stocks,setRatings,running]);
...
function NewsPanel({tickers}){
  ...
  useEffect(()=>{const top10=tickers.slice(0,10).join(',');fetch(`/api/news?tickers=${top10}`).then(r=>r.json()).then(d=>{setNews(d.news||[]);setLoading(false);}).catch(()=>setLoading(false));},[]);
...
function InsidersPanel({tickers}){
  const[insiders,setInsiders]=useState([]);const[loading,setLoading]=useState(false);const[selectedTicker,setSelectedTicker]=useState(tickers[0]);
  const load=useCallback((ticker)=>{setLoading(true);setInsiders([]);fetch(`/api/insiders?ticker=${ticker}`).then(r=>r.json()).then(d=>{setInsiders(d.insiders||[]);setLoading(false);}).catch(()=>setLoading(false));},[]);
  useEffect(()=>{load(selectedTicker);},[selectedTicker]);
  const buys=insiders.filter(t=>t.type==='BUY');const sells=insiders.filter(t=>t.type==='SELL');
  return<div>
    <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:20}}>
      {tickers.slice(0,15).map(t=><button key={t} onClick={()=>setSelectedTicker(t)} style={{padding:'4px 12px',borderRadius:20,border:`1px solid ${selectedTicker===t?G.accent:G.border}`,background:selectedTicker===t?G.accent+'22':'transparent',color:selectedTicker===t?G.accent:G.muted,cursor:'pointer',fontSize:12,fontWeight:600}}>{t}</button>)}
    </div>
    ...
    {!loading&&insiders.length>0&&<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:20}}>
      {[{label:'Compras',data:buys,color:G.green},{label:'Vendas',data:sells,color:G.red}].map(({label,data,color})=>(
        <div key={label}>
          ...
          {data.map((t,i)=><div key={i} style={{padding:'10px 12px',marginBottom:8,background:color+'11',borderRadius:8,border:`1px solid ${color}22`}}>
            <div style={{display:'flex',justifyContent:'space-between'}}>
              <span style={{color:G.text,fontSize:13,fontWeight:600}}>{t.name||'Insider'}</span>
              <span style={{color,fontSize:13,fontWeight:700}}>{t.type==='BUY'?'+':'-'}${fmtK(t.value)}</span>
            </div>
            <div style={{color:G.muted,fontSize:12,marginTop:4}}>{t.shares?.toLocaleString()} acções{t.price>0&&` · $${t.price.toFixed(2)}/acção`}{t.date&&` · ${t.date}`}</div>
          </div>)}
        </div>
      ))}
    </div>}
...
function GrowthChart({portfolio}){
  ...
  const SECTOR_CAGR={Technology:{bear:0.06,base:0.14,bull:0.22},Semiconductor:{bear:0.07,base:0.16,bull:0.26},Consumer:{bear:0.04,base:0.09,bull:0.14},Healthcare:{bear:0.03,base:0.08,bull:0.14},Energy:{bear:0.02,base:0.06,bull:0.12},ETF:{bear:0.05,base:0.10,bull:0.16},Default:{bear:0.04,base:0.10,bull:0.17}};
  const getSector=(ticker)=>{const t=ticker.toUpperCase();if(['GOOGL','MSFT','META','AAPL','NVDA','HOOD','HIMS','NFLX','FRSH','TEM','INFQ'].includes(t))return'Technology';if(['MRVL','IONQ','AXTI','SMH','POET'].includes(t))return'Semiconductor';if(['AMZN','KO','OSCR'].includes(t))return'Consumer';if(['ABCL','NBIS'].includes(t))return'Healthcare';if(['KBR','PH','MOS'].includes(t))return'Energy';if(['GOLD'].includes(t))return'ETF';return'Default';};
  let bearCAGR=0,baseCAGR=0,bullCAGR=0;
  portfolio.forEach(p=>{const w=(p.value||0)/totalValue;const cagr=SECTOR_CAGR[getSector(p.ticker)]||SECTOR_CAGR.Default;bearCAGR+=w*cagr.bear;baseCAGR+=w*cagr.base;bullCAGR+=w*cagr.bull;});
  ...
  const maxVal=bullVals[10];const W=580,H=260,PL=70,PR=20,PT=20,PB=40;
  const chartW=W-PL-PR;const chartH=H-PT-PB;
  const xPos=(i)=>PL+(i/10)*chartW;const yPos=(v)=>PT+chartH-(v/maxVal)*chartH;
  const pathD=(vals)=>vals.map((v,i)=>`${i===0?'M':'L'} ${xPos(i)} ${yPos(v)}`).join(' ');
  const fmtEur=(v)=>v>=1e6?'€'+(v/1e6).toFixed(1)+'M':v>=1e3?'€'+(v/1e3).toFixed(0)+'K':'€'+v.toFixed(0);
  return<div>
    <div style={{color:G.text,fontSize:14,fontWeight:600,marginBottom:12}}>Projecção de Crescimento (10 anos)</div>
    <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%',maxWidth:W}}>
      {[0.25,0.5,0.75,1].map(p=>{const y=PT+chartH-p*chartH;return<g key={p}><line x1={PL} x2={W-PR} y1={y} y2={y} stroke={G.border} strokeWidth={1}/><text x={PL-6} y={y+4} fill={G.muted} fontSize={10} textAnchor="end">{fmtEur(maxVal*p)}</text></g>;})}
      ...
      <text x={xPos(10)+4} y={yPos(bearVals[10])+4} fill={G.red} fontSize={11}>{fmtEur(bearVals[10])}</text>
      <text x={xPos(10)+4} y={yPos(baseVals[10])-4} fill={G.accent} fontSize={12} fontWeight="bold">{fmtEur(baseVals[10])}</text>
      <text x={xPos(10)+4} y={yPos(bullVals[10])-4} fill={G.green} fontSize={11}>{fmtEur(bullVals[10])}</text>
    </svg>
...
function OverviewPanel({portfolio,ratings}){
  ...
  const getSector=(ticker)=>{const t=ticker.toUpperCase();if(['GOOGL','MSFT','META','AAPL','NVDA','HOOD','HIMS','NFLX','FRSH','TEM','INFQ','WYFI'].includes(t))return'Technology';if(['MRVL','IONQ','AXTI','SMH','POET'].includes(t))return'Semiconductors';if(['AMZN','KO','OSCR'].includes(t))return'Consumer';if(['ABCL','NBIS'].includes(t))return'Healthcare';if(['KBR','PH','MOS','TMC'].includes(t))return'Industrials/Materials';if(['GOLD'].includes(t))return'Commodities/ETF';if(['DXYZ','IREN','OUST','XPEV'].includes(t))return'Special/Growth';return'Other';};
  const sectorMap={};portfolio.forEach(p=>{const s=getSector(p.ticker);if(!sectorMap[s])sectorMap[s]=0;sectorMap[s]+=p.value||0;});
  ...
  const alerts=[];const topStock=portfolio[0];
  ...
  if(tech/totalValue>0.6)alerts.push({msg:`Exposição a Tech/Semis: ${((tech/totalValue)*100).toFixed(0)}% — considerar diversificação setorial.`});
...
function DiscoverPanel({portfolio}){
  const[suggestions,setSuggestions]=useState([]);const[loading,setLoading]=useState(false);const[done,setDone]=useState(false);const[error,setError]=useState(null);
  const handleDiscover=async()=>{setLoading(true);setError(null);try{const res=await fetch('/api/discover',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({portfolio})});const data=await res.json();if(data.error)setError(data.error);else setSuggestions(data.suggestions||[]);setDone(true);}catch(e){setError(e.message);}setLoading(false);};
  return<div>
    <div style={{marginBottom:20}}>
      <div style={{color:G.text,fontSize:16,fontWeight:700,marginBottom:8}}>Descobertas AI ✨</div>
      <div style={{color:G.muted,fontSize:13,lineHeight:1.6}}>A IA analisa o teu portfolio e sugere 5 novos stocks que complementam as tuas posições actuais — diversificando gaps sectoriais, adicionando exposição a tendências de crescimento e equilibrando risco/retorno.</div>
    </div>
    ...
    {loading&&<div style={{textAlign:'center',padding:40}}><div style={{color:G.accent,fontSize:15}}>A analisar portfolio e mercados...</div><div style={{color:G.muted,fontSize:13,marginTop:8}}>Isto pode demorar 15-30 segundos</div></div>}
    {error&&<div style={{color:G.red,padding:16,background:G.red+'11',borderRadius:8,marginTop:16}}>Erro: {error}</div>}
    {done&&suggestions.length>0&&<div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:16}}>
        {suggestions.map((s,i)=>{
          ...
          const cagrColor=s.cagr3y>=20?G.green:s.cagr3y>=10?G.yellow:G.muted;
          return<Card key={i} style={{padding:20,position:'relative',overflow:'hidden'}}>
            ...
            <div style={{paddingLeft:12}}>
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12,marginBottom:12}}>
                <div>
                  <div style={{display:'flex',alignItems:'center',gap:10}}><span style={{color:G.text,fontSize:18,fontWeight:800}}>{s.ticker}</span><span style={{color:G.muted,fontSize:13}}>{s.name}</span><Pill color={ratingMeta.color} bg={ratingMeta.bg}>{ratingMeta.label}</Pill></div>
                ...
                <div style={{textAlign:'right',minWidth:90}}>
                  ...
                  <div style={{fontSize:22,fontWeight:800,color:cagrColor}}>+{s.cagr3y}%</div>
                  <div style={{height:4,background:G.border,borderRadius:2,marginTop:4}}><div style={{height:'100%',borderRadius:2,width:`${Math.min(s.cagr3y*2.5,100)}%`,background:cagrColor}}/></div>
                </div>
              </div>
...
function PortfolioTable({portfolio,ratings,onAnalyse,filterRating}){
  const filtered=filterRating==='Todos'?portfolio:portfolio.filter(p=>{const rMap={'Strong Buy':'STRONG_BUY','Buy':'BUY','Hold':'HOLD','Sell':'SELL','Strong Sell':'STRONG_SELL'};return ratings[p.ticker]===rMap[filterRating];});
  return<div style={{display:'flex',flexDirection:'column',gap:8}}>
    {filtered.map(stock=>{const rating=ratings[stock.ticker];const meta=rating?RATING_META[rating]:null;
      return<div key={stock.ticker} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 16px',background:G.card,border:`1px solid ${G.border}`,borderRadius:10}}>
        <div style={{minWidth:60}}><div style={{color:G.text,fontSize:14,fontWeight:700}}>{stock.ticker}</div><div style={{color:G.muted,fontSize:11,marginTop:1}}>{stock.name?.slice(0,18)}</div></div>
        <div style={{flex:1}}><div style={{color:G.text,fontSize:14,fontWeight:600}}>{fmt(stock.value)}</div><div style={{color:stock.pnl>=0?G.green:G.red,fontSize:12}}>{fmt(stock.pnl)} ({fmtPct(stock.pnlPct)})</div></div>
...
export default function MainApexApp(){
  const[portfolio]=useState(PORTFOLIO_DEFAULT);const[ratings,setRatings]=useState({});const[activeTab,setActiveTab]=useState('Portfolio');const[selectedStock,setSelectedStock]=useState(null);const[filterRating,setFilterRating]=useState('Todos');
  useEffect(()=>{fetch('/api/ratings').then(r=>r.json()).then(d=>{if(d.ratings)setRatings(d.ratings);}).catch(()=>{});},[]);
  const totalValue=portfolio.reduce((s,p)=>s+(p.value||0),0);
  const totalPnL=portfolio.reduce((s,p)=>s+(p.pnl||0),0);
  ...
  return<div style={{background:G.bg,minHeight:'100vh',color:G.text,fontFamily:"'DM Sans',sans-serif",padding:'20px 16px'}}>
    <div style={{maxWidth:900,margin:'0 auto'}}>
      <div style={{marginBottom:24}}>
        ...
        {Object.entries(ratingGroups).some(([,v])=>v.length>0)&&<div style={{display:'flex',gap:12,marginTop:12,flexWrap:'wrap'}}>
          {Object.entries(ratingGroups).map(([r,tks])=>{const meta=RATING_META[r];return<div key={r} style={{fontSize:12,color:meta.color}}><span style={{fontWeight:700}}>{meta.label}</span>{tks.length>0?<span style={{color:G.muted}}> {tks.join(', ')}</span>:<span style={{color:G.muted}}> Sem posições.</span>}</div>;})}
        </div>}
      </div>
      <div style={{display:'flex',gap:4,marginBottom:24,borderBottom:`1px solid ${G.border}`,paddingBottom:0}}>
        {tabs.map(tab=><button key={tab} onClick={()=>setActiveTab(tab)} style={{background:'none',border:'none',cursor:'pointer',padding:'10px 16px',fontSize:13,fontWeight:600,color:activeTab===tab?G.accent:G.muted,borderBottom:activeTab===tab?`2px solid ${G.accent}`:'2px solid transparent',transition:'all 0.2s',display:'flex',alignItems:'center',gap:6}}>
      ...
      {activeTab==='Portfolio'&&<div>
        <div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap'}}>
          {['Todos','Strong Buy','Buy','Hold','Sell','Strong Sell'].map(f=><button key={f} onClick={()=>setFilterRating(f)} style={{padding:'4px 12px',borderRadius:20,border:`1px solid ${filterRating===f?G.accent:G.border}`,background:filterRating===f?G.accent+'22':'transparent',color:filterRating===f?G.accent:G.muted,cursor:'pointer',fontSize:12,fontWeight:600}}>{f}</button>)}
        </div>
