    ...
    <div style={{ marginTop: 16, padding: 16, background: '#0d0d1a', borderRadius: 10, border: `1px solid ${G.border}` }}>
      ...
      {total > 0 && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ display: 'flex', height: 8, borderRadius: 4, overflow: 'hidden', gap: 1 }}>
            {bars.map(b => b.count > 0 && (
              <div key={b.key} style={{
                ...
                background: RATING_META[b.key].color,
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 6 }}>
            {bars.map(b => (
              <div key={b.key} style={{ fontSize: 11, color: b.count > 0 ? RATING_META[b.key].color : G.muted }}>
                {b.label} {b.count}
              </div>
            ))}
      ...
      {priceTargetMean > 0 && (
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: 11, color: G.muted }}>Alvo Médio</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: G.text }}>${priceTargetMean?.toFixed(2)}</div>
          </div>
          <div>
            <div style={{ fontSize: 11, color: G.muted }}>Alto</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: G.green }}>${priceTargetHigh?.toFixed(2)}</div>
          </div>
          <div>
            <div style={{ fontSize: 11, color: G.muted }}>Baixo</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: G.red }}>${priceTargetLow?.toFixed(2)}</div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: 28, maxWidth: 700, width: '100%', maxHeight: '85vh',
        overflowY: 'auto', position: 'relative'
      }}>
        <button onClick={onClose} style={{
          position: 'absolute', top: 16, right: 16, background: 'none',
          border: 'none', color: G.muted, cursor: 'pointer', fontSize: 20
        }}>✕</button>
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ color: G.muted, fontSize: 13, marginTop: 4 }}>
            {fmt(stock.value)} · {fmtPct(stock.pnlPct)} · {stock.weight?.toFixed(1)}% do portfolio
          </div>
        </div>
        <AnalystPanel ticker={stock.ticker} />
        {loading && (
          <div style={{ textAlign: 'center', padding: 40 }}>
            <div style={{ color: G.accent, fontSize: 14 }}>A analisar com IA 360°...</div>
            <div style={{ color: G.muted, fontSize: 12, marginTop: 8 }}>Recolhendo dados de mercado, analistas e contexto setorial</div>
          </div>
        )}
        ...
        {analysis && !loading && (
          <div style={{ marginTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              {analysis.confidence && (
                <span style={{ color: G.muted, fontSize: 12 }}>Confiança: {analysis.confidence}</span>
              )}
            </div>
            <div style={{
              color: G.text, fontSize: 14, lineHeight: 1.7,
              whiteSpace: 'pre-wrap', background: '#0d0d1a',
            ...
            {analysis.keyRisks && (
              <div style={{ marginTop: 12 }}>
                <div style={{ color: G.yellow, fontSize: 12, fontWeight: 600, marginBottom: 6 }}>RISCOS</div>
                <div style={{ color: G.muted, fontSize: 13 }}>{analysis.keyRisks}</div>
              </div>
            )}
            {analysis.catalyst && (
              <div style={{ marginTop: 12 }}>
                <div style={{ color: G.green, fontSize: 12, fontWeight: 600, marginBottom: 6 }}>CATALISADORES</div>
                <div style={{ color: G.muted, fontSize: 13 }}>{analysis.catalyst}</div>
              </div>
            )}
          </div>
        )}
  ...
  try {
    ...
    if (data.results) {
      ...
      await fetch('/api/ratings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ratings: map })
      });
    }
  } catch (e) {
...
function BatchAnalyser({ stocks, ratings, setRatings }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      ...
      {running && (
        <div style={{ color: G.accent, fontSize: 13 }}>
          A analisar {stocks.length} stocks com IA 360°...
        </div>
      )}
      {done && (
        <div style={{ color: G.green, fontSize: 13 }}>
          ✓ {rated} stocks classificados
          <button onClick={() => { hasRun.current = false; setDone(false); handleRun(); }}
            style={{ marginLeft: 8, background: 'none', border: `1px solid ${G.border}`, color: G.muted, borderRadius: 6, padding: '2px 8px', cursor: 'pointer', fontSize: 11 }}>
            Reanalisar
          </button>
        </div>
      )}
...
function NewsPanel({ tickers }) {
  ...
  useEffect(() => {
    const top10 = tickers.slice(0, 10).join(',');
    fetch(`/api/news?tickers=${top10}`)
      .then(r => r.json())
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div style={{ flex: 1 }}>
                <a href={item.url} target="_blank" rel="noopener noreferrer"
                  style={{ color: G.text, textDecoration: 'none', fontSize: 14, fontWeight: 600, lineHeight: 1.4, display: 'block' }}>
                  {item.headline}
                </a>
                <div style={{ display: 'flex', gap: 8, marginTop: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {item.tickers?.map(t => (
                    <Pill key={t} color={G.accent}>{t}</Pill>
                  ))}
                  {item.sentiment !== undefined && (
                    <span style={{ fontSize: 12, color: sentColor }}>
                      {item.sentiment > 0.2 ? '▲ Positivo' : item.sentiment < -0.2 ? '▼ Negativo' : '● Neutro'}
                    </span>
                  )}
                </div>
              </div>
...
function InsidersPanel({ tickers }) {
  ...
  const load = useCallback((ticker) => {
    ...
    fetch(`/api/insiders?ticker=${ticker}`)
      .then(r => r.json())
      .then(d => { setInsiders(d.insiders || []); setLoading(false); })
      .catch(() => setLoading(false));
  }, []);

  useEffect(() => { load(selectedTicker); }, [selectedTicker]);

  const buys = insiders.filter(t => t.type === 'BUY');
  const sells = insiders.filter(t => t.type === 'SELL');
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 20 }}>
        {tickers.slice(0, 15).map(t => (
          <button key={t} onClick={() => setSelectedTicker(t)} style={{
            padding: '4px 12px', borderRadius: 20, border: `1px solid ${selectedTicker === t ? G.accent : G.border}`,
            background: selectedTicker === t ? G.accent + '22' : 'transparent',
            color: selectedTicker === t ? G.accent : G.muted,
            cursor: 'pointer', fontSize: 12, fontWeight: 600
          }}>{t}</button>
        ))}
      </div>
      <div style={{ color: G.muted, fontSize: 12, marginBottom: 16 }}>
        Filtro: transações {'>'} $25K ou {'>'} 500 acções · últimos 180 dias
      </div>
      {loading && <div style={{ color: G.muted, textAlign: 'center', padding: 40 }}>A carregar insiders...</div>}
      {!loading && !insiders.length && (
        <div style={{ color: G.muted, textAlign: 'center', padding: 40 }}>
          Sem transações relevantes para {selectedTicker}.
        </div>
      )}
      {!loading && insiders.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
          {[{ label: 'Compras', data: buys, color: G.green }, { label: 'Vendas', data: sells, color: G.red }].map(({ label, data, color }) => (
            <div key={label}>
              <div style={{ color, fontSize: 13, fontWeight: 700, marginBottom: 10 }}>
                {label} ({data.length})
              </div>
                ...
                <div key={i} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <span style={{ color, fontSize: 13, fontWeight: 700 }}>
                      {t.type === 'BUY' ? '+' : '-'}${fmtK(t.value)}
                    </span>
                  </div>
                  <div style={{ color: G.muted, fontSize: 12, marginTop: 4 }}>
                    {t.shares?.toLocaleString()} acções
                    {t.price > 0 && ` · $${t.price.toFixed(2)}/acção`}
                    {t.date && ` · ${t.date}`}
                  </div>
                </div>
              ))}
...
function GrowthChart({ portfolio }) {
  ...
  const SECTOR_CAGR = {
    ...
    Finance:       { bear: 0.04, base: 0.09, bull: 0.15 },
  ...
  const getSector = (ticker) => {
    ...
    if (['GOOGL','MSFT','META','AAPL','NVDA','HOOD','HIMS','NFLX','FRSH','TEM','INFQ'].includes(t)) return 'Technology';
    ...
    if (['KBR','PH','MOS'].includes(t)) return 'Energy';
  ...
  portfolio.forEach(p => {
    ...
    const cagr = SECTOR_CAGR[sector] || SECTOR_CAGR.Default;
    bearCAGR += w * cagr.bear;
    baseCAGR += w * cagr.base;
    bullCAGR += w * cagr.bull;
  });
  ...
  const W = 580, H = 260, PL = 70, PR = 20, PT = 20, PB = 40;
  ...
  return (
    <div>
      <div style={{ color: G.text, fontSize: 14, fontWeight: 600, marginBottom: 12 }}>
        Projecção de Crescimento (10 anos)
      </div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', maxWidth: W }}>
        {[0.25, 0.5, 0.75, 1].map(p => {
          ...
          return (
            <g key={p}>
              <line x1={PL} x2={W - PR} y1={y} y2={y} stroke={G.border} strokeWidth={1} />
              <text x={PL - 6} y={y + 4} fill={G.muted} fontSize={10} textAnchor="end">
                {fmtEur(maxVal * p)}
              </text>
            </g>
          );
        ...
        {[0, 2, 4, 6, 8, 10].map(y => (
          <text key={y} x={xPos(y)} y={H - PB + 16} fill={G.muted} fontSize={10} textAnchor="middle">
            {y === 0 ? 'Hoje' : `+${y}a`}
          </text>
        ))}
        <path
          d={`${pathD(bullVals)} L ${xPos(10)} ${yPos(bearVals[10])} ${bearVals.map((v,i) => `${i===0?'L':'L'} ${xPos(10-i)} ${yPos(v)}`).join(' ')} Z`}
          fill={G.green} fillOpacity={0.06}
        />
      ...
      <div style={{ display: 'flex', gap: 20, marginTop: 8 }}>
        {[
          { label: `Bear (${(bearCAGR*100).toFixed(0)}% CAGR)`, color: G.red },
          { label: `Base (${(baseCAGR*100).toFixed(0)}% CAGR)`, color: G.accent },
          { label: `Bull (${(bullCAGR*100).toFixed(0)}% CAGR)`, color: G.green },
        ].map(({ label, color }) => (
...
function OverviewPanel({ portfolio, ratings }) {
  ...
  const getSector = (ticker) => {
    ...
    if (['AMZN','KO','OSCR'].includes(t)) return 'Consumer';
    if (['ABCL','NBIS'].includes(t)) return 'Healthcare';
    if (['KBR','PH','MOS','TMC'].includes(t)) return 'Industrials/Materials';
  ...
  portfolio.forEach(p => {
    const r = ratings[p.ticker];
    if (r && ratingCounts[r] !== undefined) ratingCounts[r]++;
  });
  const ratedTotal = Object.values(ratingCounts).reduce((s, v) => s + v, 0);

  const alerts = [];
  const topStock = portfolio[0];
  if (topStock && (topStock.value / totalValue) > 0.35) {
    alerts.push({ type: 'warning', msg: `${topStock.ticker} representa ${((topStock.value/totalValue)*100).toFixed(0)}% do portfolio — peso elevado.` });
  }
  const tech = (sectorMap['Technology'] || 0) + (sectorMap['Semiconductors'] || 0);
  if (tech / totalValue > 0.6) {
    alerts.push({ type: 'warning', msg: `Exposição a Tech/Semis: ${((tech/totalValue)*100).toFixed(0)}% — considerar diversificação setorial.` });
  }
  const losers = portfolio.filter(p => p.pnlPct < -15);
  if (losers.length > 0) {
    alerts.push({ type: 'info', msg: `${losers.length} posição(ões) com P&L < -15%: ${losers.map(p => p.ticker).join(', ')}` });
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      {alerts.length > 0 && (
        <Card style={{ padding: 16 }}>
          ...
          {alerts.map((a, i) => (
            <div key={i} style={{ color: G.muted, fontSize: 13, padding: '6px 0', borderBottom: i < alerts.length - 1 ? `1px solid ${G.border}` : 'none' }}>
              {a.msg}
            </div>
          ))}
        </Card>
          ...
          return (
            <div key={name} style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                ...
                <span style={{ color: G.muted, fontSize: 13 }}>{fmt(val)} · {pct.toFixed(1)}%</span>
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        {[{ label: '🏆 Top Performers', data: winners, color: G.green }, { label: '📉 Piores Performers', data: topLosers, color: G.red }].map(({ label, data, color }) => (
          <Card key={label} style={{ padding: 16 }}>
            <div style={{ color: G.text, fontSize: 13, fontWeight: 600, marginBottom: 12 }}>{label}</div>
            {data.map(p => (
              <div key={p.ticker} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: `1px solid ${G.border}` }}>
      ...
      {ratedTotal > 0 && (
        <Card style={{ padding: 16 }}>
          <div style={{ color: G.text, fontSize: 14, fontWeight: 600, marginBottom: 16 }}>Distribuição de Ratings IA</div>
            ...
            {Object.entries(ratingCounts).map(([r, count]) => {
              ...
              return (
                <div key={r} style={{ textAlign: 'center', padding: '10px 16px', background: meta.bg, borderRadius: 10, border: `1px solid ${meta.color}33` }}>
                  <div style={{ color: meta.color, fontSize: 20, fontWeight: 800 }}>{count}</div>
...
function DiscoverPanel({ portfolio }) {
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <div style={{ color: G.text, fontSize: 16, fontWeight: 700, marginBottom: 8 }}>
          Descobertas AI ✨
        </div>
        <div style={{ color: G.muted, fontSize: 13, lineHeight: 1.6 }}>
          A IA analisa o teu portfolio e sugere 5 novos stocks que complementam as tuas posições actuais —
          diversificando gaps sectoriais, adicionando exposição a tendências de crescimento e equilibrando risco/retorno.
        </div>
      </div>
      {!done && !loading && (
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: 40 }}>
          <div style={{ color: G.accent, fontSize: 15 }}>A analisar portfolio e mercados...</div>
          <div style={{ color: G.muted, fontSize: 13, marginTop: 8 }}>Isto pode demorar 15-30 segundos</div>
        </div>
      )}
      {error && (
        <div style={{ color: G.red, padding: 16, background: G.red + '11', borderRadius: 8, marginTop: 16 }}>
          Erro: {error}
        </div>
      )}
      {done && suggestions.length > 0 && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <button onClick={() => { setDone(false); setSuggestions([]); }} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 12 }}>
                      ...
                      <div style={{ textAlign: 'right', minWidth: 90 }}>
                        ...
                        <div style={{ height: 4, background: G.border, borderRadius: 2, marginTop: 4 }}>
                          <div style={{
                            height: '100%', borderRadius: 2,
                            width: `${Math.min(s.cagr3y * 2.5, 100)}%`,
                            background: cagrColor
                          }} />
                        </div>
                      </div>
                    </div>
      ...
      {done && suggestions.length === 0 && !error && (
        <div style={{ color: G.muted, padding: 40, textAlign: 'center' }}>
          Sem sugestões disponíveis. Tenta regenerar.
        </div>
      )}
    </div>
  );
...
function PortfolioTable({ portfolio, ratings, onAnalyse, filterRating }) {
  const filtered = filterRating === 'Todos'
    ? portfolio
    : filterRating === 'Sem Rating'
      ? portfolio.filter(p => !ratings[p.ticker])
      : portfolio.filter(p => {
          const rMap = { 'Strong Buy': 'STRONG_BUY', 'Buy': 'BUY', 'Hold': 'HOLD', 'Sell': 'SELL', 'Strong Sell': 'STRONG_SELL' };
          return ratings[p.ticker] === rMap[filterRating];
        });
        ...
        return (
          <div key={stock.ticker} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: stock.pnl >= 0 ? G.green : G.red, fontSize: 12 }}>
                {fmt(stock.pnl)} ({fmtPct(stock.pnlPct)})
              </div>
            </div>
            <div style={{ color: G.muted, fontSize: 12, minWidth: 50, textAlign: 'right' }}>
            ...
            <button onClick={() => onAnalyse(stock)} style={{
              background: G.accent + '22', border: `1px solid ${G.accent}44`,
              color: G.accent, borderRadius: 8, padding: '4px 12px',
              cursor: 'pointer', fontSize: 12, fontWeight: 600
            }}>
              Analisar
            </button>
...
export default function MainApexApp() {
  ...
  portfolio.forEach(p => {
    const r = ratings[p.ticker];
    if (r && ratingGroups[r]) ratingGroups[r].push(p.ticker);
  });
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
            <div>
              <h1 style={{ margin: 0, fontSize: 24, fontWeight: 800, color: G.text, letterSpacing: '-0.5px' }}>
                Apex <span style={{ color: G.accent }}>/</span> Intel
              </h1>
            </div>
          ...
          <div style={{ display: 'flex', gap: 20, marginTop: 16, flexWrap: 'wrap' }}>
            <div>
              <div style={{ color: G.muted, fontSize: 11 }}>Total</div>
              <div style={{ color: G.text, fontSize: 18, fontWeight: 700 }}>{fmt(totalValue)}</div>
            </div>
            <div>
              <div style={{ color: G.muted, fontSize: 11 }}>P&L</div>
              <div style={{ color: totalPnL >= 0 ? G.green : G.red, fontSize: 18, fontWeight: 700 }}>
                {totalPnL >= 0 ? '+' : ''}{fmt(totalPnL)}
              </div>
            </div>
            <div>
              <div style={{ color: G.muted, fontSize: 11 }}>Rated</div>
              <div style={{ color: G.text, fontSize: 18, fontWeight: 700 }}>{ratedCount}/{portfolio.length}</div>
            </div>
          </div>

          {Object.entries(ratingGroups).some(([, v]) => v.length > 0) && (
            <div style={{ display: 'flex', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
              {Object.entries(ratingGroups).map(([r, tickers]) => {
                const meta = RATING_META[r];
                return (
                  <div key={r} style={{ fontSize: 12, color: meta.color }}>
                    <span style={{ fontWeight: 700 }}>{meta.label}</span>
                    {tickers.length > 0 ? (
                      <span style={{ color: G.muted }}> {tickers.join(', ')}</span>
                    ) : (
                      <span style={{ color: G.muted }}> Sem posições.</span>
                    )}
                  </div>
                );
              })}
        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 24, borderBottom: `1px solid ${G.border}`, paddingBottom: 0 }}>
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              color: activeTab === tab ? G.accent : G.muted,
              borderBottom: activeTab === tab ? `2px solid ${G.accent}` : '2px solid transparent',
              ...
              {tab === 'Descobertas' && (
                <span style={{
                  ...
                  background: G.purple + '33', color: G.purple,
                  borderRadius: 4, border: `1px solid ${G.purple}44`
                }}>AI</span>
              )}
            </button>
          ))}
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
              {['Todos', 'Strong Buy', 'Buy', 'Hold', 'Sell', 'Strong Sell'].map(f => (
                <button key={f} onClick={() => setFilterRating(f)} style={{
                  padding: '4px 12px', borderRadius: 20, border: `1px solid ${filterRating === f ? G.accent : G.border}`,
                  background: filterRating === f ? G.accent + '22' : 'transparent',
                  color: filterRating === f ? G.accent : G.muted,
                  cursor: 'pointer', fontSize: 12, fontWeight: 600
                }}>{f}</button>
              ))}
            </div>
        ...
        {selectedStock && (
          <AnalysisPanel stock={selectedStock} onClose={() => setSelectedStock(null)} />
        )}
      </div>
    </div>
  );
